...
A Codex plugin that ships two standalone skills for running `codex exec` prompts on a schedule:
...
The daemon (or a one-shot `run-due`) selects due tasks and executes each as:
...
node --test skills/schedule/scripts/schedule.test.mjs # 40 tests
```
