...
**Codex 프롬프트를 위한 cron.**
터미널에서 로컬로, 안전하게, 어떤 `codex exec` 작업이든 반복하거나 예약하세요.
...
- **의존성 없음** — 순수 Node ESM과 `node:test`만 사용합니다. 안전성 및 잠금 경쟁 adversarial 케이스를 포함해 73개 테스트가 통과합니다(loop 33개, schedule 40개).
...
- `daemon` — 간격별 polling. `--once`, `--max-runs N`, `--poll-ms`, `--codex-bin`
