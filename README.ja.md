...
任意の `codex exec` ジョブを、ターミナルからローカルかつ安全にループ実行またはスケジュール実行できます。
...
- **依存関係ゼロ** — 純粋な Node ESM と `node:test`。安全性とロック競合の adversarial ケースを含む 73 テストが通っています（loop 33、schedule 40）。
...
- `daemon` — 一定間隔で polling。`--once`、`--max-runs N`、`--poll-ms`、`--codex-bin`
